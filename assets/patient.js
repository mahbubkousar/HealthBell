...
import { collection, query, where, getDocs, doc, getDoc, updateDoc, writeBatch, addDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js";
...
async function handleDoseTaken(prescriptionId, medIndex, medName, doseTime) {
    ...
    try {
        ...
        if (newRemainingCount <= 5 && newRemainingCount > 0) { // Check if stock is low but not yet zero
             console.log(`LOW STOCK WARNING for ${medName}! Remaining: ${newRemainingCount}. Notifying pharmacy...`);
...
function generateTodaysMedication(prescriptions, todaysLogs) {
    ...
    const morningMeds = [], noonMeds = [], nightMeds = [];
            ...
            const createDoseHtml = (doseTime) => {
                ...
                return `<div class="flex items-center justify-between p-3 bg-gray-50 rounded-md">
                        ...
                        <label class="flex items-center cursor-pointer"><input type="checkbox" class="form-checkbox h-5 w-5 text-blue-600 take-dose-cb" 
                                   ${hasTaken ? 'checked disabled' : ''} data-prescription-id="${pres.id}" data-medicine-index="${index}" data-medicine-name="${med.name}" data-dose-time="${doseTime}">
                            <span class="ml-2 text-sm font-medium ${hasTaken ? 'text-green-600' : 'text-gray-700'}">${hasTaken ? 'Taken' : 'Mark as Taken'}</span></label></div>`;
            };
    ...
    const buildSection = (title, medsArray) => {
        ...
        return `<div class="mb-4"><h3 class="text-lg font-bold text-gray-600 border-b pb-2 mb-3">${title}</h3><div class="space-y-2">${medsArray.join('')}</div></div>`;
    };

    const finalHtml = buildSection('☀️ Morning', morningMeds) + buildSection('🕛 Noon', noonMeds) + buildSection('🌙 Night', nightMeds);
    todaysMedsList.innerHTML = finalHtml.trim() === '' ? '<p class="text-center text-gray-500">No medication scheduled for today, or stock needs to be added.</p>' : finalHtml;
}
...
function renderFullPrescriptions(prescriptions) {
    ...
    prescriptions.forEach(pres => {
        ...
        pres.medicines.forEach((med, index) => {
            ...
            medicinesHtml += `<li class="p-3 bg-gray-50 rounded-md border"><div class="flex justify-between items-center"><div><strong>${med.name}</strong> - for ${med.totalDays} days<br><span class="text-sm text-gray-600">Dose: ${doses.join(', ')}</span></div>${stockHtml}</div></li>`;
        });
        medicinesHtml += '</ul>';
        card.innerHTML = `<div class="flex justify-between items-start"><div><h3 class="text-xl font-bold text-gray-800">Prescription from Dr. ${pres.doctorName}</h3><p class="text-sm text-gray-500">Date: ${pres.startDate}</p></div><span class="text-sm font-semibold py-1 px-3 rounded-full ${pres.status === 'active' ? 'bg-green-200 text-green-800' : 'bg-gray-200 text-gray-800'}">${pres.status}</span></div><div class="mt-4"><h4 class="font-semibold text-gray-700">Medicines:</h4>${medicinesHtml}</div>`;
